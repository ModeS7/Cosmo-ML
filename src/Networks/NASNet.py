...
class NASNet(nn.Module):
    def __init__(self, num_stem_features, num_normal_cells, filters, scaling, skip_reduction, use_aux=True,
        ...
        self.conv0 = nn.Sequential(OrderedDict([
            ('conv', nn.Conv2d(1, num_stem_features, kernel_size=3, stride=2, bias=False)),
        ...
        for i in range(3):
            ...
            if i == 1 and self.use_aux:
                ...
                self.aux_linear = nn.Linear(19200, num_outputs)
            # scaling
        ...
        self.linear = nn.Linear(x_channels, self.num_outputs)  # large: 4032; mobile: 1056
...
def NASNetAMobile(num_outputs=1000):
    return NASNet(32, 4, 44, 2, skip_reduction=False, use_aux=True, num_outputs=num_outputs)


def NASNetALarge(num_outputs=1000):
    return NASNet(96, 6, 168, 2, skip_reduction=True, use_aux=True, num_outputs=num_outputs)
